...
const FIELD_MAX_LEN = {
  ...
  nfa_trust_name: 200, non_nfa_trust_name: 200, nfa_type: 100, nfa_form_type: 50,
...
function jsonRecordToFirearm(r) {
  ...
  return {
    ...
    non_nfa_trust_name: r.non_nfa_trust_name || null,
...
router.get('/settings/export/full', requireAuth, requireAdmin, (req, res) => {
  ...
  const firearms = firearmsQueries.all().map(f => firearmsQueries.findById(f.id));
...
router.post('/settings/import', requireAuth, requireAdmin, (req, res) => {
  importUpload(req, res, (err) => {
    ...
    for (const record of records) {
      try { firearmsQueries.create(record); imported++; }
      catch (e) { importErrors.push(e.message); }
    }
